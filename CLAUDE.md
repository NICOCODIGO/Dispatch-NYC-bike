...
- **Do not move the score by anything that does not print its own line on the receipt.** Demand, time of day and weather were banned outright for a long time, on the grounds that they make the number more accurate and much harder to audit — and a dispatcher who cannot audit a number will not act on it. The ban has been narrowed to what it was actually protecting. Such a term is allowed when it is a *named, visible* contribution in `ScoreBreakdown.factors` — `Winter mode −10`, `Transit hub ×1.2` — rendered by the receipt exactly like `capacityWeight` is. What stays forbidden is the same thing as before: a term that silently shifts the number, so two people looking at one station see different scores and neither can say why. Auditability was always the requirement; "no demand signals" was only the cheapest way to guarantee it.
